...
export default function (ctx: Ctx) {
	return (
		<div class="flex flex-col space-y-4">
			<span class="text-neutral-400 font-medium">Elgato Key Light</span>
			...
			<Switch>
				...
				<Match when={ctx.state() === "discovering"}>
					<div class="flex flex-row items-center space-x-4">
						<span class="text-yellow-400">Scanning network for Elgato Key Lights...</span>
					</div>
				</Match>
					...
					<div class="flex flex-col space-y-2">
						...
						<ul class="text-sm text-neutral-500 space-y-1">
							<For each={[...ctx.devices().values()]}>
								{(device) => (
									<li>
										{device.name || device.id} ({device.addr}:{device.port})
									</li>
								)}
							</For>
						</ul>
