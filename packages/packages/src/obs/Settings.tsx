...
export default function (ctx: Ctx) {
	...
	return (
		<>
			<Show when={getRemoteShellMode() && ctx.hostMirrorRows().length > 0}>
				<table class="mb-4 table-auto w-full text-white">
					...
					<tbody>
						<For each={ctx.hostMirrorRows()}>
							{(row) => (
								<tr>
									<td>{row.url}</td>
									<td>
										{row.state}
										{row.hasPassword ? " · password on host" : ""}
									</td>
								</tr>
							)}
						</For>
				...
				<p class="text-sm text-neutral-500 mb-4">
					OBS runs on the host; this list is read-only on remote.
				</p>
			</Show>
			...
			<Show when={!getRemoteShellMode() || ctx.hostMirrorRows().length === 0}>
				...
				>
					<fieldset
						disabled={form.state.isSubmitting || getRemoteShellMode()}
						class="space-y-4"
					>
