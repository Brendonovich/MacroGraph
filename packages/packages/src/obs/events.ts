...
export function alignmentConversion(alignment: string | number, types: Types) {
  switch (alignment) {
    case "Center":
      return 0;
    case "Bottom Center":
    ...
    case "Top Right":
      return 6;
    case 0:
      return types.Alignment.variant("Center");
    case 8:
      return types.Alignment.variant("Bottom Center");
    case 9:
      return types.Alignment.variant("Bottom Left");
    case 10:
      return types.Alignment.variant("Bottom Right");
    case 2:
      return types.Alignment.variant("Center Right");
    case 1:
      return types.Alignment.variant("Center Left");
    case 4:
      return types.Alignment.variant("Top Center");
    case 5:
      return types.Alignment.variant("Top Left");
    case 6:
      return types.Alignment.variant("Top Right");
  }
}
...
export function register(pkg: Package<EventTypes>, types: Types) {
    ...
    createIO: ({ io }) => {
      return {
        ...
        scenes: io.dataOutput({
          id: "scenes",
          name: "Scenes",
          type: t.list(t.struct(types.Scenes)),
        }),
      };
    },
    run({ ctx, data, io }) {
    ...
    createIO: ({ io }) => {
      return {
        ...
        inputAudioTracks: io.dataOutput({
          ...
          type: t.struct(types.AudioTracks),
        }),
      };
    },
    run({ ctx, data, io }) {
      ctx.setOutput(io.inputName, data.inputName);
      const audioTracks = types.AudioTracks.create({
  ...
  createOBSEventSchema({
    ...
    run({ ctx, data, io }) {
      ...
      ctx.setOutput(
        io.monitorType,
        types.MonitorType.variant(
          data.monitorType as InferEnum<typeof types.MonitorType>["variant"],
        ),
      );
      ctx.exec(io.exec);
    },
    ...
    createIO: ({ io }) => {
      return {
        ...
        inputs: io.dataOutput({
          id: "inputs",
          name: "Inputs",
          type: t.list(t.struct(types.InputVolumeMeter)),
        }),
      };
    },
    run({ ctx, data, io }) {
      const volumeMeters = data.inputs.map((data) =>
        types.InputVolumeMeter.create({
