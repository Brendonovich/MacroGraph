...
import { Context, Effect, Layer, type Option, type Schema as S } from "effect";
import type { EffectGenerator, IO } from "@macrograph/project-domain";

export namespace PackageEngine {
	export type Any = PackageEngine<any, any, any, any, Resource.Tag<any, any>>;
...
export namespace Schema {
	...
	export type InferIO<IO> = IO extends IO.DataInput<infer T>
		...
		: IO extends IO.DataOutput<infer T>
			...
			: IO extends IO.ExecOutput
				? IO
				: IO extends IO.ExecInput
					? never
	...
	> extends Metadata<Id, Properties> {
		...
		}) => EffectGenerator<Effect.Effect<void | IO.ExecOutput>>;
	}
