											...
											onSchemaClicked={(schema) => {
												...
												const pin = Solid.batch(() => {
													...
													if (
														...
														if (suggestion.pin instanceof ExecOutput) {
															const execIn = node.state.inputs.find(
																(input) => input instanceof ExecInput,
															);
															if (execIn) {
																graph().connectPins(suggestion.pin, execIn);
																return execIn;
															}
														} else if (suggestion.pin instanceof ExecInput) {
															const execOut = node.state.outputs.find(
																(input) => input instanceof ExecOutput,
															);
															if (execOut) {
																graph().connectPins(execOut, suggestion.pin);
																return execOut;
															}
														} else if (suggestion.pin instanceof DataOutput) {
															const suggestionPin = suggestion.pin;
															const dataIn = node.state.inputs.find(
																(input): input is DataInput<any> =>
																	input instanceof DataInput &&
																	renderType(input.type) ===
																		renderType(suggestionPin.type),
															);
															if (dataIn) {
																graph().connectPins(suggestion.pin, dataIn);
																return dataIn;
															}
														} else if (suggestion.pin instanceof DataInput) {
															const suggestionPin = suggestion.pin;
															const dataOut = node.state.outputs.find(
																(output): output is DataOutput<any> =>
																	output instanceof DataOutput &&
																	renderType(output.type) ===
																		renderType(suggestionPin.type),
															);
															if (dataOut) {
																graph().connectPins(dataOut, suggestion.pin);
																return dataOut;
															}
														}
													}
